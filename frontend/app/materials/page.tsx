...
import { Plus, Trash2, Package, Edit2, X } from "lucide-react";
...
export default function MaterialPage() {
  ...
  const [formData, setFormData] = useState({ "code": "", "description": "", "material_type": "", "default_uom": "" });
  ...
  useEffect(() => {
    fetchData();
    fetch('http://localhost:3001/uom').then(res => res.json()).then(data => setUoms(data)).catch(console.error);
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const url = editId ? `http://localhost:3001/materials/${editId}` : `http://localhost:3001/materials`;
      ...
      if (res.ok) {
        setFormData({ "code": "", "description": "", "material_type": "", "default_uom": "" });
        setEditId(null);
        fetchData();
      } else {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`http://localhost:3001/materials/${id}`, { method: "DELETE" });
      if (res.ok) fetchData();
    } catch (err) {
      console.error(err);
    }
    ...
    <div className="flex flex-col gap-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white shadow-sm ring-1 ring-gray-900/5 p-5 rounded-xl border border-gray-200 h-fit sticky top-24">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">{editId ? "Edit Material" : "Add New Material"}</h2>
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-gray-700 uppercase tracking-wider">Description</label>
              <input
                required
                type="text"
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-gray-700 uppercase tracking-wider">Type</label>
              <input
                required
                type="text"
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-gray-700 uppercase tracking-wider">Default UOM</label>
              <select
                required
                value={formData.default_uom}
                onChange={e => setFormData({ ...formData, default_uom: e.target.value })}
                className="bg-white border border-gray-300 rounded-lg px-3 py-2 text-sm text-gray-900 focus:outline-none focus:border-indigo-500 transition-colors appearance-none"
              >
            ...
            {editId && (
              <button
                type="button"
                onClick={() => { setEditId(null); setFormData({ "code": "", "description": "", "material_type": "", "default_uom": "" }); }}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-gray-50 border-b border-gray-200 text-gray-500 uppercase tracking-wider text-xs">
                <tr>
                  <th className="px-6 py-4 font-medium">Code</th><th className="px-6 py-4 font-medium">Description</th><th className="px-6 py-4 font-medium">Type</th><th className="px-6 py-4 font-medium">Default UOM</th>
                ...
                {loading ? (
                  ...
                  data.map((item, idx) => (
                    <tr key={item.id || idx} className="hover:bg-gray-50 hover:bg-gray-100 transition-colors">
                      <td className="px-6 py-4 text-gray-700">{item.code}</td><td className="px-6 py-4 text-gray-700">{item.description}</td><td className="px-6 py-4 text-gray-700">{item.material_type}</td><td className="px-6 py-4 text-gray-700">{item.default_uom}</td>
                      <td className="px-6 py-4 text-right flex items-center justify-end gap-1">
                        <button
                          onClick={() => {
                            setEditId(item.id);
                            setFormData({ code: item.code, description: item.description, material_type: item.material_type, default_uom: item.default_uom });
                          }}

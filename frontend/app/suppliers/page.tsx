...
import { Plus, Trash2, Truck, Edit2, X } from "lucide-react";
...
export default function SupplierPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const url = editId ? `http://localhost:3001/suppliers/${editId}` : `http://localhost:3001/suppliers`;
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`http://localhost:3001/suppliers/${id}`, { method: "DELETE" });
      if (res.ok) fetchData();
    } catch (err) {
      console.error(err);
    }
    ...
    <div className="flex flex-col gap-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white shadow-sm ring-1 ring-gray-900/5 p-5 rounded-xl border border-gray-200 h-fit sticky top-24">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">{editId ? "Edit Supplier" : "Add New Supplier"}</h2>
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-gray-700 uppercase tracking-wider">Contact Info</label>
              <input
                required
                type="text"
            ...
            {editId && (
              <button
                type="button"
                onClick={() => { setEditId(null); setFormData({ "name": "", "contact_info": "" }); }}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-gray-50 border-b border-gray-200 text-gray-500 uppercase tracking-wider text-xs">
                <tr>
                  <th className="px-6 py-4 font-medium">Name</th><th className="px-6 py-4 font-medium">Contact Info</th>
                ...
                {loading ? (
                  ...
                  data.map((item, idx) => (
                    <tr key={item.id || idx} className="hover:bg-gray-50 hover:bg-gray-100 transition-colors">
                      <td className="px-6 py-4 text-gray-700">{item.name}</td><td className="px-6 py-4 text-gray-700">{item.contact_info}</td>
                      <td className="px-6 py-4 text-right flex items-center justify-end gap-1">
                        <button
                          onClick={() => {
                            setEditId(item.id);
                            setFormData({ name: item.name, contact_info: item.contact_info });
                          }}

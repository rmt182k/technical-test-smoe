...
export default function CreateRequest() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("http://localhost:3001/request-material", {
        method: "POST",
      ...
      if (res.ok) {
        ...
        alert(`Error: ${errorData.message || 'Failed to create request'}`);
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="max-w-4xl mx-auto flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900">New Request</h1>
          <p className="text-gray-500 mt-1">Create a new material request</p>
        </div>
      </div>
        ...
        <div className="bg-white shadow-sm ring-1 ring-gray-900/5 p-6 rounded-xl border border-gray-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex flex-col gap-2">
              ...
              <input required type="text" value={formData.code} onChange={e => setFormData({ ...formData, code: e.target.value })} className="bg-white border border-gray-300 rounded-lg px-4 py-2.5 text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all" placeholder="e.g. REQ-2026-001" />
            </div>
            <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-col gap-2 md:col-span-2">
              ...
              <textarea value={formData.notes} onChange={e => setFormData({ ...formData, notes: e.target.value })} className="bg-white border border-gray-300 rounded-lg px-4 py-2.5 text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all h-24 resize-none" placeholder="Provide reason for request..." />
            </div>
          </div>
        </div>
              ...
              <div key={index} className="p-4 bg-gray-50 hover:bg-gray-100 rounded-lg border border-gray-200 relative group">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input required type="text" value={item.material_type} onChange={e => handleItemChange(index, 'material_type', e.target.value)} className="bg-white border border-gray-300 rounded-md px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-1 focus:ring-indigo-500" placeholder="e.g. IT, Office" />
                  </div>
                  <div className="flex flex-col gap-1.5">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input type="text" value={item.currency} onChange={e => handleItemChange(index, 'currency', e.target.value)} className="bg-white border border-gray-300 rounded-md px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-1 focus:ring-indigo-500" placeholder="IDR, USD" />
                  </div>
                </div>
              </div>
            ))}

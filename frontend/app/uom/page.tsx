...
import { Plus, Trash2, Ruler, Edit2, X } from "lucide-react";
...
export default function UOMPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await fetch(`http://localhost:3001/uom`);
      if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const url = editId ? `http://localhost:3001/uom/${editId}` : `http://localhost:3001/uom`;
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`http://localhost:3001/uom/${id}`, { method: "DELETE" });
      if (res.ok) fetchData();
    } catch (err) {
      console.error(err);
    }
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-500">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900">Unit of Measure</h1>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white shadow-sm ring-1 ring-gray-900/5 p-5 rounded-xl border border-gray-200 h-fit sticky top-24">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">{editId ? "Edit UOM" : "Add New UOM"}</h2>
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            {editId && (
              <button
                type="button"
                onClick={() => { setEditId(null); setFormData({ "code": "", "description": "" }); }}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-gray-50 border-b border-gray-200 text-gray-500 uppercase tracking-wider text-xs">
                <tr>
                  <th className="px-6 py-4 font-medium">Code</th><th className="px-6 py-4 font-medium">Description</th>
                ...
                {loading ? (
                  ...
                  data.map((item, idx) => (
                    <tr key={item.id || idx} className="hover:bg-gray-50 hover:bg-gray-100 transition-colors">
                      <td className="px-6 py-4 text-gray-700">{item.code}</td><td className="px-6 py-4 text-gray-700">{item.description}</td>
